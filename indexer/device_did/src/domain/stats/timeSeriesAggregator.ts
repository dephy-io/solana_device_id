...
export class AccessTimeSeriesAggregator {
  aggregate(
    curr: DeviceDidEvent | AccessTimeStats,
  ...
  protected processAccessStats(
    ...
    if ((curr as DeviceDidEvent).timestamp) {
      const event = curr as DeviceDidEvent
      let signer: string
  ...
  protected isDeviceDidEvent(
    event: DeviceDidEvent | AccessTimeStats,
  ): event is DeviceDidEvent {
