...
export default class DeviceDidDiscoverer {
  ...
  async loadAccounts(): Promise<DeviceDidAccountInfo[]> {
    const newAccounts: DeviceDidAccountInfo[] = []
   ...
   */
  async getAllAccounts(): Promise<DeviceDidAccountInfo[]> {
    const connection = solanaPrivateRPC.getConnection()
    const accountsInfo: DeviceDidAccountInfo[] = []
