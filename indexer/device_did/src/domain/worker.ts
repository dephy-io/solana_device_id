  ...
  implements SolanaIndexerWorkerDomainI, IndexerWorkerDomainWithStats
{
  ...
  constructor(
    ...
    protected programId = DEVICE_DID_PROGRAM_ID,
  ) {
    super(context)
  }

  async onNewAccount(
    config: AccountIndexerConfigWithMeta<DeviceDidAccountInfo>,
  ): Promise<void> {
  ...
  async getStats(
    account: string,
  ): Promise<AccountStats<DeviceDidAccountStats>> {
  ...
  async getAccountInfo(actual: string): Promise<DeviceDidAccountInfo> {
