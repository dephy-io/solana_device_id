...
export class StatsAggregator {
    async aggregate(args) {
        ...
        const last24hEvents = await timeSeriesDAL.getAllValuesFromTo([...commonFields, currHour.minus({ hours: 24 }).toMillis()], [...commonFields, currHour.toMillis()]);
        let last24h;
        for await (const event of last24hEvents) {
            last24h = accessAggregator.aggregate(event.data, last24h);
        }
        const last7dEvents = await timeSeriesDAL.getAllValuesFromTo([...commonFields, currHour.minus({ hours: 24 * 7 }).toMillis()], [...commonFields, currHour.toMillis()]);
        let last7d;
        ...
        if (last1h)
            stats.last1h = last1h.data;
        if (last24h)
            stats.last24h = last24h;
        if (last7d)
            stats.last7d = last7d;
        if (total)
            stats.total = total.data;
        stats.accessingPrograms = new Set([
